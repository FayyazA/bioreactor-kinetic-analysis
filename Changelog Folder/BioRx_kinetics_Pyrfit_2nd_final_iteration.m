function [X,Sfit_dyn,results]=BioRx_kinetics_Pyrfit_2nd_final_iteration(S_dyn,filename)
...
X0 = [1/48,0.01,1,...    %1/T1P, Kpl,FlowPyr=0.15
...
% Inital conditions
    ...
    K  = [-1/48.61-x(2)-x(3)] ; %Pyr=P(-R1P-kpl-Flow)                  Setting Rate Constant of dpyr = k * pyr
...
opts = optimset('MaxIter',2000,'MaxFunEvals', 1e32,'TolX',1e-9,'TolFun', abs(min(abs(S_dyn(:,end)))/1e15),'FinDiffType','central','Display','iter');

lb = [1/51, .0001, 10^-8,...
      X0(4)-2, 8, 1e3];
 ub = [1/47,.1,10.0,...
      X0(4)+2, 25, 1e10];%used to be 22
