function result = run_multiple_files_Tissue_slices_lactate_sum_with_fraction()
...
pyr_table = zeros(12,5);
lacin_table = zeros(12,9);
...
for k = 1 : length(theFiles)
    ...
    [C,S_fitdyn2] = Tissue_slices_with_fraction(my_array,X,strcat(theFiles(k),"LacTissue"),cell_line); %[round2_output,fit2,results2] = BioRx_kinetics_Lacin_NoT1Lin(my_array,round1_output,strcat(theFiles(k),"Lacincs"));
    lacin_table(k,:) = C;
...
xlswrite("Pyr_summaryconditional7.xlsx",pyr_table)
xlswrite("LacTissue0222celldataconditional7.xlsx",lacin_table)
