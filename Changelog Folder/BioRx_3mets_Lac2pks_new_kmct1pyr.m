function [E,Sfit_dyn]=BioRx_3mets_Lac2pks_new_kmct1pyr(S_dyn,A,C,filename)
...
X0 = [C(1), C(2), C(3),... % 1/T1Pyr, Kpl, Flow_pyr
    ...
    1/36.7, 0, 0]; % 1/T1Lout,Flow_lacout, K(MCT1)
    
function Mest_dyn = model_exchange_dyn(x)
% Inital conditions
    Mest_dyn(:,1) = S_dyn(:,1);
    K  = [-C(1)-x(2)-0.5-x(3),x(5),0;... %Pyr=P(-R1P-kpl-Flow-KMCT1Pyr)+Lac_in(Klp) Replace flow by 0.5 and put KMCT1 in entry(1,1)
          x(2),-x(4)-x(5)-x(6),x(9);...   %Lac_in=P(kpl)+Lac_in(-R1PL-klp-K(MCT4))+Lac_out(K(MCT1))
          0,x(6), -x(7)-0.5-x(9)] ;   %Lac_ex=Lac_in(K(MCT4))+Lac_out(-R1Lac_out-Flow-K(MCT1))   
...
lb = [1/51, .0001, 10^-20,... % 1/T1Pyr, Kpl, Flow_pyr Flow_pyr used to be C(3)*0.9 1/T1P was C(1)-.001
      C(4)-.001, 0, 0.001,... %1/T1Lin, Klp, K(MCT4)
      1/37.7, 0, 0]; % 1/T1Lout,Flow_lacout, K(MCT1)
 ub = [1/47, 0.08, 10.0,... % 1/T1Pyr, Kpl, Flow_pyr Flow_pyr used to be C(3)*1.1 1/T1P was C(1)-.001
      C(4)+.001, 0.1, 10.0,... %1/T1Lin, Klp, K(MCT4)
      1/35.7, 0, .9 ]; % 1/T1Lout,Flow_lacout, K(MCT1)
