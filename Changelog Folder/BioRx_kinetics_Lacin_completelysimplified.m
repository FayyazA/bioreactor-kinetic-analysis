function [X,Sfit_dyn,result_vector] = BioRx_kinetics_Lacin_completelysimplified(S_dyn,A,filename)
...
X0 = [A(1),0.007264...    %1/T1Pyr, Kpl, Flow
     .14929]; %1/T1Lin, K(MCT4)
...
% Inital conditions
    Mest_dyn(:,1) = S_dyn(:,1);
       ...
       K  = [-x(1),0;... %Pyr=P(-R1P-kpl-Flow)
          x(2),-x(3)];   %Lac_in=P(kpl)+Lac_in(-K(MCT4)-RT1Lin)   
...
lb =  [0.026,0.001,...          %[A(1)-0.01,0.001,...          %A(1)-.0001, 0.0001,0.0001,... %0.005,...   %.01,...                                      %0,A(3)-.05,...                    
      1/35+1/20];      %0.05];         %0.005];      %.01];            %1/35, .01];
 ub = [1/45+0.8,0.5,...          %[A(1)+0.01,0.05,...         %A(1)+.0001,0.08,0.3... %0.5,0.2,...  %0.08,0.15,...   %.3,...                                                     %A(2)+.1,A(3)+.05,...                   
      2.1];          %0.2];         %2];%10];     %2.1];      %2];                %1/10, 2];
...
result_vector = [T1Pyr_Kpl_Flow,Kpl,K_MCT4_T1_Lin_term,Rsqrd_pyr_total,Rsqrd_lac_in_magn,rmse_pyr,rmse_lac_in];
