...
for k = 1 : length(theFiles)
    ...
    [X,S_fit_dyn,results,lb,ub] = BioRx_kinetics_Pyrfit_starting_out_recursive(input_vector,strcat(theFiles(k),"PyrFINALsmooth"),lb,ub);
    lb_table(k,:) = lb;
    ub_table(k,:) = ub;
    ...
    [C,S_fitdyn2,lb2,ub2] = BioRx_Pyr_Lacin_recursive(my_array,X,strcat(theFiles(k),"LacinFINALsmooth"),lb2,ub2); %[round2_output,fit2,results2] = BioRx_kinetics_Lacin_NoT1Lin(my_array,round1_output,strcat(theFiles(k),"Lacincs"));
    lacin_table(k,:) = C;
    lb2_table(k,:) = lb2;
    ub2_table(k,:) = ub2;
    ...
    [E,Sfitdyn3,lb3,ub3] = BioRx_3mets_Lac2pks_tight_Fl_bounds_recursive(my_array_2,X,C,strcat(theFiles(k),"LacexFINALsmooth"),lb3,ub3); %[fit3,results3] = BioRx_kinetics_Lac2pks_NoT1s(my_array_2,round1_output,round2_output,strcat(theFiles(k),"Lacexcs"));
    lacex_table(k,:) = E;
    lb3_table(k,:) = lb3;
    ub3_table(k,:) = ub3;
...
xlswrite("Lacin_summary_tight_Fl_bounds3_recursive.xlsx",lacin_table)
xlswrite("Lacex_summary_tight_Fl_bounds3_recursive.xlsx",lacex_table)
xlswrite("lb_3.xlsx",lb_table)
xlswrite("lb2_3.xlsx",lb2_table)
xlswrite("lb3_3.xlsx",lb3_table)
xlswrite("ub_3.xlsx",ub_table)
xlswrite("ub2_3.xlsx",ub2_table)
xlswrite("ub3_3.xlsx",ub3_table)
