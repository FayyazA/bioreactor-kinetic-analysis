function [Sfit_dyn,result_vector]=BioRx_3mets_Lac2pks_twoflows_one_step(S_dyn,filename)
...
X0 = [1/48,0.01,0.15,... % 1/T1Pyr, Kpl, Flow_pyr
    1/25, 0, .04,... %1/T1Lin, Klp, K(MCT4)
    1/36.7, 0.15*4, 0,...
    3,19,5e8]; % 1/T1Lout,Flow_lacout, K(MCT1)
    
function Mest_dyn = model_exchange_dyn(x)
% Inital conditions
    Mest_dyn(:,1) = S_dyn(:,1);
    K  = [-1/50.67-x(2)-x(3),0,0;... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp)
          x(2),-x(4)-x(6),0;...   %Lac_in=P(kpl)+Lac_in(-R1PL-klp-K(MCT4))+Lac_out(K(MCT1))
          0,x(6), -1/31.49-x(8)] ;   %Lac_ex=Lac_in(K(MCT4))+Lac_out(-R1Lac_out-Flow-K(MCT1))   
   
      Inpfunc(:,1) = [0;0;0];

    for k = 2:length(S_dyn)
       %gamma pdf approximation of input function , func = @(a,x)( a(3)*x.^(a(1)-1) .* exp(-x/a(2)) / (a(2)^a(1) * gamma(a(1))));
       Inpfunc(:,k) =[ ( x(12)*(k*TR).^(x(10)-1) .* exp(-(k*TR)/x(11)) / (x(11)^x(10) * gamma(x(10))));...
                        0;...
                        0]; 
...
lb = [1/50, .0001, .0001,... % 1/T1Pyr, Kpl, Flow_pyr
      1/30, 0,.01,... %1/T1Lin, Klp, K(MCT4)
      1/37.7, 0.01, 0,...
      2, 16, 1e3]; % 1/T1Lout,Flow_lacout, K(MCT1)
 ub = [1/47,.1,0.3,... % 1/T1Pyr, Kpl, Flow_pyr
      1/22, 0.5,.9,... %1/T1Lin, Klp, K(MCT4)
      1/35.7, 0.9, .9,...
      6, 22, 1e10]; % 1/T1Lout,Flow_lacout, K(MCT1)
...
result_vector = [T1P,Kpl,Flow_pyr,T1Lin,Klp,K_MCT4,T1Lex,Flow_lac,K_MCT1,Rsqrd_pyr_total,Rsqrd_lac_in_magn,Rsqrd_lac_ex_magn,rmse_pyr,rmse_lac_in,rmse_lac_ex];
