function [X,Sfit_dyn,result_vector] = BioRx_kinetics_Lacin_NoT1Lin(S_dyn,A,filename)
...
% Inital conditions
    Mest_dyn(:,1) = S_dyn(:,1);
       ...
       K  = [-1/50.67-x(1)-x(2),0;... %Pyr=P(-R1P-kpl-Flow)
          x(1),-x(3)-x(4)];   %Lac_in=P(kpl)+Lac_in(-K(MCT4)-RT1Lin)   
...
lb = [0.0001,0.01,...           %0.0001,...           %[A(1)-.0001, 0.0001,0.0001,... %0.005,...   %.01,...                                      %0,A(3)-.05,...                    
     1/35+0.001,1/35];           %0.005];      %.01];            %1/35, .01];
 ub = [0.5,0.25,...         %0.5,...          %[A(1)+.0001,0.08,0.3... %0.5,0.2,...  %0.08,0.15,...   %.3,...                                                     %A(2)+.1,A(3)+.05,...                   
     10.1,1/10];           %2];%10];     %2.1];      %2];                %1/10, 2];
...
result_vector = [Kpl,Flow,K_MCT4,T1Lin,Rsqrd_pyr_total,Rsqrd_lac_in_magn,rmse_pyr,rmse_lac_in];
