function [result_vector,Sfit_dyn]=Tissue_slices_noKlp(S_dyn,A,filename)
...
% Inital conditions
    Mest_dyn(:,1) = S_dyn(:,1);
       ...
       K  = [-x(1)-x(2)-x(3),0;... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp)
          x(2),-x(4)-x(5)];   %Lac_in=P(kpl)+Lac_in(-klp-1/T1Lin-Flow_Lac)   
