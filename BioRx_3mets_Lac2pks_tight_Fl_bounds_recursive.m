function [E,Sfit_dyn,lb,ub]=BioRx_3mets_Lac2pks_tight_Fl_bounds_recursive(S_dyn,A,C,filename,lb,ub)
...
% (from Keshari & WIlson (2014) T1 @ 14.1 T for Lac: 32.7 s , DH @ 11.7t =34.8 � 0.5)  
% Flow=0.081 to 0.27 
...
X0 = [C(1), C(2), C(3),... % 1/T1Pyr, Kpl, Flow_pyr
    ...
    1/36.7, 0.5, 0]; % 1/T1Lout,Flow_lacout, K(MCT1)
...
% Inital conditions
    ...
    for k = 2:length(S_dyn)
       ...
       Mest_dyn(:,k) = expm(K*k*TR)*Mest_dyn(:,1)*cos(flip*pi/180);% +Inpfunc(:,k-1);
    end
end
%%
...
%      C(4)-.001, 0, 0.00001,... %1/T1Lin, Klp, K(MCT4)
%      1/37.7, 0.3, 0]; % 1/T1Lout,Flow_lacout=10^-8, K(MCT1)
 %ub = [1/47, 0.08, 10.0,... % 1/T1Pyr, Kpl, Flow_pyr Flow_pyr used to be C(3)*1.1 1/T1P was C(1)-.001
 %     C(4)+.001, 0.5, 10.0,... %1/T1Lin, Klp, K(MCT4)
 %     1/35.7, 0.7, .9 ]; % 1/T1Lout,Flow_lacout=10, K(MCT1)
...
marker = "False";
% if abs(X(1) - lb(1)) <= 0.00001 * X(1)
% lb(1) = lb(1) - 0.5 * lb(1);
% marker = "True";
% disp(1)
% end
% if abs(X(1) - ub(1)) <= 0.00001 * X(1)
% ub(1) = ub(1) + 0.5 * ub(1);
% marker = "True";
% disp(1)
% end
% if abs(X(2) - lb(2)) <= 0.00001 * X(2)
% lb(2) = lb(2) - 0.5 * lb(2);
% marker = "True";
% disp(2)
% end
% if abs(X(2) - ub(2)) <= 0.00001 * X(2)
% ub(2) = ub(2) + 0.5 * ub(2);
% marker = "True";
% disp(2)
% end
if abs(X(3) - lb(3)) <= 0.00001 * X(3)
lb(3) = lb(3) - 0.5 * lb(3);
marker = "True";
disp(3)
end
if abs(X(3) - ub(3)) <= 0.00001 * X(3)
ub(3) = ub(3) + 0.5 * ub(3);
marker = "True";
disp(3)
end
marker = "False";
%if abs(X(4) - lb(4)) <= 0.00001 * X(4)
%lb(4) = lb(4) - 5 * lb(4);
%marker = "True";
%disp(4)
%end
%if abs(X(4) - ub(4)) <= 0.00001 * X(4)
%ub(4) = ub(4) + 5 * ub(4);
%marker = "True";
%disp(4)
%end
if abs(X(5) - lb(5)) <= 0.00001 * X(5)
lb(5) = lb(5) - 0.5 * lb(5);
marker = "True";
disp(5)
end
if abs(X(5) - ub(5)) <= 0.00001 * X(5)
ub(5) = ub(5) + 0.5 * ub(5);
marker = "True";
disp(5)
end
% if abs(X(6) - lb(6)) <= 0.00001 * X(6)
% lb(6) = lb(6) - 0.5 * lb(6);
% marker = "True";
% disp(6)
% end
% if abs(X(6) - ub(6)) <= 0.00001 * X(6)
% ub(6) = ub(6) + 0.5 * ub(6);
% marker = "True";
% disp(6)
% end
%if abs(X(7) - lb(7)) <= 0.00001 * X(7)
%lb(7) = lb(7) - 0.5 * lb(7);
%marker = "True";
%disp(7)
%end
%if abs(X(7) - ub(7)) <= 0.00001 * X(7)
%ub(7) = ub(7) + 0.5 * ub(7);
%marker = "True";
%disp(7)
%end
if abs(X(8) - lb(8)) <= 0.00001 * X(8)
lb(8) = lb(8) - 0.5 * lb(8);
marker = "True";
disp(8)
end
if abs(X(8) - ub(8)) <= 0.00001 * X(8)
ub(8) = ub(8) + 0.5 * ub(8);
marker = "True";
disp(8)
end
% if abs(X(9) - lb(9)) <= 0.00001 * X(9)
% lb(9) = lb(9) - 0.5 * lb(9);
% marker = "True";
% disp(9)
% end
% if abs(X(9) - ub(9)) <= 0.00001 * X(9)
% ub(9) = ub(9) + 0.5 * ub(9);
% marker = "True";
% disp(9)
% end
...
if marker == "True"
    [E,Sfit_dyn,lb,ub]=BioRx_3mets_Lac2pks_tight_Fl_bounds_recursive(S_dyn,A,C,filename,lb,ub)
end
