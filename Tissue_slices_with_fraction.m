function [result_vector,Sfit_dyn]=Tissue_slices_with_fraction(S_dyn,A,filename,cell_line)
...
X0 = [A(1),A(2),A(3),...    %1/T1Pyr, Kpl, Flow_pyr
    0, 0.5]; %klp, Fraction_intracellular NOTE: OLD STARTING POINT FOR T1L WAS 1/20, WORKED BETTER
...
% Inital conditions
   ...
   if cell_line == "HK2"
       K  = [-x(1)-x(2)-x(3),x(4);... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp)
          x(2),-x(4)-(1/24.7678)*x(5) - (1/37.5664) * (1-x(5)) - 0.15402 * (1-x(5)) - 0.08295 * (1-x(5))];   %Lac_in=P(kpl)+Lac_in(-klp-1/T1Lin-Flow_Lac)   
   elseif cell_line == "UMRC6"
       K  = [-x(1)-x(2)-x(3),x(4);... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp)
          x(2),-x(4)-(1/23.9395)*x(5) - (1/36.3622) * (1-x(5)) - 0.6458 * (1-x(5)) - 0.51631 * (1-x(5))];
   elseif cell_line == "UOK262"
       K  = [-x(1)-x(2)-x(3),x(4);... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp)
          x(2),-x(4)-(1/21.6977)*x(5) - (1/36.7184) * (1-x(5)) - 0.96399 * (1-x(5)) - 1.84875 * (1-x(5))];
   end
...
lb = [1/51,X0(2)-0.001, 10^-8,... %A(1)-.0001
      10^-20,0];
 ub = [1/47, X0(2)+0.001,25,... %A(1)+.0001
     .1, 1];
...
result_vector = [X(1),X(2),X(3),X(4),X(5),R(1),R(2),rmse(1),rmse(2)];
figure
subplot(2,1,1)
