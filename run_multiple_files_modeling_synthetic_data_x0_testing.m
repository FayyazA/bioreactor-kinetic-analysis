function table_data = run_multiple_files_modeling_synthetic_data_x0_testing(data_vec,filename,xknot,kinetic_parameter)
...
table_data = zeros(1,18);
    ...
    [X,S_fit_dyn] = BioRx_kinetics_Pyrfit_starting_out_starting_point_increments(input_vector,strcat(filename,"PyrFinalConfirmation"),xknot,kinetic_parameter);
    %pyr_table(k,:) = results1;
    %A_table(k,:) = round1_output;
    my_array = data_vec(1:2,:);
    my_array(2,:) = smooth(data_vec(2,:));
    [C,S_fitdyn2] = BioRx_Pyr_Lacin_starting_point_increments(my_array,X,strcat(filename,"LacinFinalConfirmation"),xknot,kinetic_parameter); %[round2_output,fit2,results2] = BioRx_kinetics_Lacin_NoT1Lin(my_array,round1_output,strcat(theFiles(k),"Lacincs"));
    %lacin_table(k,:) = C;
    %C_table(k,:) = round2_output;
    my_array_2 = data_vec;
    my_array_2(2,:) = smooth(data_vec(2,:));
    my_array_2(3,:) = smooth(data_vec(3,:));
    [E,Sfitdyn3] = BioRx_3mets_Lac2pks_starting_point_increments(my_array_2,X,C,strcat(filename,"LacexFinalConfirmation"),xknot,kinetic_parameter); %[fit3,results3] = BioRx_kinetics_Lac2pks_NoT1s(my_array_2,round1_output,round2_output,strcat(theFiles(k),"Lacexcs"));
    table_data = E;
