function [Sfit_dyn,result_vector] = BioRx_kinetics_Lac2pks_NoT1s(S_dyn,A,C,filename)
...
X0 = [C(1), C(2), C(3),... % 1/T1Pyr, Kpl, Flow
    C(4),... %1/T1Lin, K(MCT4)_T1Lin_term
    0.4,1]; % 1/T1Lout
...
% Inital conditions
    ...
    K  = [-x(1)-x(2)-x(3),0,0;... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp)
          x(2),-x(4),0;...   %Lac_in=P(kpl)+Lac_in(-R1PL-K(MCT4))
          0,x(4) * x(5), -(x(3) * x(6))] ;   %Lac_ex=Lac_in(K(MCT4))+Lac_out(-R1Lac_out-Flow)   
...
lb = [1/55,0.001,0.005,...           %[C(1)-.001, C(2)-.02,C(3)-0.02,...%.0001, C(3)*0.9,... % 1/T1Pyr, Kpl, Flow_pyr
     1/35+0.001,10^-3/(10^-3+.1),...           %C(4)-0.2,0.01,...%C(4)-.001, 0.001,... %1/T1Lin, K(MCT4)
     (0.3+1/45)/0.3];           %0.01];%1/45,]; 1/40,]; % 1/T1Lout,Flow_lacout, K(MCT1) Changed from 38.5 to 39
 ub = [1/48,0.5,0.3,...          %[C(1)+.001, C(2)+0.02,C(3)+0.02,...%0.08, C(3)*1.1,... % 1/T1Pyr, Kpl, Flow_pyr
      10.1,350/351,...          %C(4)+0.2,1,...%C(4)+.001, 2,... %1/T1Lin, K(MCT4)
      9];          %1];%1/25,]; %1/32,]; % 1/T1Lout,Flow_lacout, K(MCT1) Changed from 35.7 to 35
...
output
T1P=(1/X(1))
KMCT4_T1Lin_Term = X(4)
F_and_T1Lex_coefficient = X(6)
T1Lin_coefficient = X(5)
Kpl=(X(2))
Flow_pyr=X(3)
...
result_vector = [T1P,Kpl,Flow_pyr,KMCT4_T1Lin_Term,F_and_T1Lex_coefficient,T1Lin_coefficient,A(4),A(5),A(6),Rsqrd_pyr_total,Rsqrd_lac_in_magn,Rsqrd_lac_ex_magn,rmse_pyr,rmse_lac_in,rmse_lac_ex];
