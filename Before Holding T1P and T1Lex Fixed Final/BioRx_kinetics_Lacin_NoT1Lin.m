...
% Inital conditions
    Mest_dyn(:,1) = S_dyn(:,1);
       ...
       K  = [-x(1)-x(2)-x(3),0;... %Pyr=P(-R1P-kpl-Flow)
          x(2),-x(4)];   %Lac_in=P(kpl)+Lac_in(-K(MCT4)-RT1Lin)   
...
lb = [1/55,0.001,0.005,...           %[A(1)-.0001, 0.0001,0.0001,... %0.005,...   %.01,...                                      %0,A(3)-.05,...                    
     1/35+1/20];           %0.005];      %.01];            %1/35, .01];
 ub = [1/48,0.5,0.3,...          %[A(1)+.0001,0.08,0.3... %0.5,0.2,...  %0.08,0.15,...   %.3,...                                                     %A(2)+.1,A(3)+.05,...                   
     10.1];           %2];%10];     %2.1];      %2];                %1/10, 2];
...
result_vector = [T1Pyr,Kpl,Flow,K_MCT4_T1_Lin_term,Rsqrd_pyr_total,Rsqrd_lac_in_magn,rmse_pyr,rmse_lac_in];
