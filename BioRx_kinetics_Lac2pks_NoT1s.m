...
X0 = [C(1), C(2),... % 1/T1Pyr, Kpl, Flow
    C(3),... %1/T1Lin, K(MCT4)_T1Lin_term
    C(4)]; % 1/T1Lout
...
% Inital conditions
    ...
    K  = [-1/50.67-x(1)-x(2),0,0;... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp)
          x(1),-x(3)-x(4),0;...   %Lac_in=P(kpl)+Lac_in(-R1PL-K(MCT4))
...
lb = [0.0001,0.001,...    %0.001,...           %[C(1)-.001, C(2)-.02,C(3)-0.02,...%.0001, C(3)*0.9,... % 1/T1Pyr, Kpl, Flow_pyr
     1/35+0.001,1/35]           %C(4)-0.2,0.01,...%C(4)-.001, 0.001,... %1/T1Lin, K(MCT4)
 ...
 ub = [0.5,0.5,...         %0.2,...     %0.5,...          %[C(1)+.001, C(2)+0.02,C(3)+0.02,...%0.08, C(3)*1.1,... % 1/T1Pyr, Kpl, Flow_pyr
      10.1,1/10]          %C(4)+0.2,1,...%C(4)+.001, 2,... %1/T1Lin, K(MCT4)
...
result_vector = [Kpl,Flow_pyr,KMCT4,T1Lin,A(4),A(5),A(6),Rsqrd_pyr_total,Rsqrd_lac_in_magn,Rsqrd_lac_ex_magn,rmse_pyr,rmse_lac_in,rmse_lac_ex];
