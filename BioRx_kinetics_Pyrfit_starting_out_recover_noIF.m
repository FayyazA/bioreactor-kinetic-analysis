function [X,Sfit_dyn,results]=BioRx_kinetics_Pyrfit_starting_out_recover_noIF(S_dyn,filename)
...
% Inital conditions
    ...
    for k = 2:length(S_dyn)
       ...
       Mest_dyn(:,k) = expm(K*TR)*Mest_dyn(:,k-1)*cos(flip*pi/180) + 0.9 * Inpfunc(:,k-1);
    end
end
%%
