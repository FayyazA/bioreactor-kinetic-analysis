function [Sfit_dyn,result_vector] = BioRx_kinetics_Lac2pks_slightlylesssimplified(S_dyn,A,C,filename)
...
X0 = [C(1), C(2), C(3),... % 1/T1Pyr, Kpl, KMCT4_T1Lin_Term
    1/22.5,... %1/T1Lin, K(MCT4)_T1Lin_term
    0.1279]; % 1/T1Lout
...
% Inital conditions
    ...
    K  = [-x(1),0,0;... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp)
          ...
          0, x(3)-x(4), -x(5)] ;   %Lac_ex=Lac_in(K(MCT4))+Lac_out(-R1Lac_out-Flow)   
...
lb = [0.026,0.001,1/35+1/20,...           %[C(1)-.001, C(2)-.02,C(3)-0.02,...%C(3)-0.02,...         %.0001, C(3)*0.9,... % 1/T1Pyr, Kpl, Flow_pyr
     1/35,...           %0.1,0.05,... %0.1785714286,0.1142857143,...       %0.1,0.05,...      %0.01,0.01,...                 %C(4)-.001, 0.001,... %1/T1Lin, K(MCT4)
 ...
 ub = [1/45+0.8,0.5,2.1,...          %[C(1)+.001, C(2)+0.02,C(3)+0.02,...%0.08, C(3)*1.1,... % 1/T1Pyr, Kpl, Flow_pyr
     1/10,...           %2,0.15,...  %0.2143,0.15,...       %2,0.15,...%0.5,0.15,...%0.2,0.15,...      %2,2,...                       %C(4)+.001, 2,... %1/T1Lin, K(MCT4)
...
result_vector = [T1P_Kpl_Flow,Kpl,KMCT4_T1Lin_term,one_overT1Lin,Flow_T1Lex_term,A(4),A(5),A(6),Rsqrd_pyr_total,Rsqrd_lac_in_magn,Rsqrd_lac_ex_magn,rmse_pyr,rmse_lac_in,rmse_lac_ex];
