function [data] = sim_rccopy(Vin, varargin)

%% Generate a circuit with the diagram above.  Description: At t = 5 ms, the switch closes, and the second capacitor becomes part of the circuit. This causes the time constant to change, which changes the capacitor voltage oscillation to change, and because of the extra capacitor, the initial amplitude changes as well.   
...
C2 = 47*10^-9;
for k = 2:50000
    Vout(k) = rc_fxn(Vin(k), Vout(k-1), dt ,R+R2, C);
end
for k = 50001:length(Vin)
    Vout(k) = rc_fxn(Vin(k),Vout(k-1),dt,R+R2,C+C2);
end
