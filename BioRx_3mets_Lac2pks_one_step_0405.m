function [E,Sfit_dyn]=BioRx_3mets_Lac2pks_one_step_0405(S_dyn,filename)
...
X0 = [1/48, 0.01, 0.15,... % 1/T1Pyr, Kpl, Flow_pyr
    1/25, 0, 0.05,... %1/T1Lin, Klp, K(MCT4)
    1/36.7, 0.6, 0,...
    (2/skewness(S_dyn(1,:)))^2,19,5e8]; % 1/T1Lout,Flow_lacout, K(MCT1)
    
function Mest_dyn = model_exchange_dyn(x)
% Inital conditions
    Mest_dyn(:,1) = S_dyn(:,1);
    K  = [-x(1)-x(2)-x(3),x(5),0;... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp) Replace flow by 0.5 and put KMCT1 in entry(1,1)
...
lb = [1/51, .0001, 1E-8,... % 1/T1Pyr, Kpl, Flow_pyr Flow_pyr used to be C(3)*0.9 1/T1P was C(1)-.001
      1/30, 0, 0.001,... %1/T1Lin, Klp, K(MCT4)
      1/37.7, 1E-8, 0,...
      X0(10)-2, 8, 1e3]; % 1/T1Lout,Flow_lacout, K(MCT1)
 ub = [1/47, 1.0, 10.0,... % 1/T1Pyr, Kpl, Flow_pyr Flow_pyr used to be C(3)*1.1 1/T1P was C(1)-.001
      1/22, 0.1, 2,... %1/T1Lin, Klp, K(MCT4)
      1/35.7, 10.0, 0.9,...
      X0(10)+2, 25, 1e10]; % 1/T1Lout,Flow_lacout, K(MCT1)
...
print(gcf,'-dtiff','-r300',strcat(filename,"kpl1_t1lin22",".tif"))
