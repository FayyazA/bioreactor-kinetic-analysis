function [E,Sfit_dyn]=BioRx_3mets_Lac2pks_FLac_constant(S_dyn,A,C,filename)
...
X0 = [C(1), C(2), C(3),... % 1/T1Pyr, Kpl, Flow_pyr
    ...
    1/36.7, 0]; % 1/T1Lout,K(MCT1)
    
function Mest_dyn = model_exchange_dyn(x)
% Inital conditions
    Mest_dyn(:,1) = S_dyn(:,1);
    K  = [-C(1)-x(2)-x(3),x(5),0;... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp)
          x(2),-x(4)-x(5)-x(6),x(8);...   %Lac_in=P(kpl)+Lac_in(-R1PL-klp-K(MCT4))+Lac_out(K(MCT1))
          0,x(6), -x(7)-x(8)-0.5] ;   %Lac_ex=Lac_in(K(MCT4))+Lac_out(-R1Lac_out-K(MCT1))   
...
E=[1/C(1), X(2), X(3),... % 1/T1Pyr, Kpl, Flow_pyr
    ...
    1/X(7), X(8),... % 1/T1Lout,Flow_lacout, K(MCT1)
...
rmse_lac_in = rmse(2)
rmse_lac_ex = rmse(3)
E = [E(1),E(2),E(3),E(4),E(5),E(6),E(7),E(8),E(9),E(10),E(11),Rsqrd_pyr_total,Rsqrd_lac_in_magn,Rsqrd_lac_ex_magn,rmse_pyr,rmse_lac_in,rmse_lac_ex];
