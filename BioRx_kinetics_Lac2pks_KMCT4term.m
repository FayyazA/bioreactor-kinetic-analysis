function [E,Sfit_dyn,result_vector] = BioRx_kinetics_Lac2pks_KMCT4term(S_dyn,A,C,filename)
...
% Inital conditions
    ...
    K  = [-x(1)-x(2)-x(3),0,0;... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp)
          ...
          0, x(5), -x(6)] ;   %Lac_ex=Lac_in(K(MCT4))+Lac_out(-R1Lac_out-Flow)   
...
lb = [C(1)-.001, C(2)-.02,C(3)-0.02,...%C(3)-0.02,...         %.0001, C(3)*0.9,... % 1/T1Pyr, Kpl, Flow_pyr
     0.1,0.05,... %0.1785714286,0.1142857143,...       %0.1,0.05,...      %0.01,0.01,...                 %C(4)-.001, 0.001,... %1/T1Lin, K(MCT4)
     0.02]; %0.15];   %0.02];                        %1/45,]; 1/40,]; % 1/T1Lout,Flow_lacout, K(MCT1) Changed from 38.5 to 39
 ub = [C(1)+.001, C(2)+0.02,C(3)+0.02,...%0.08, C(3)*1.1,... % 1/T1Pyr, Kpl, Flow_pyr
     2,0.15,...  %0.2143,0.15,...       %2,0.15,...%0.5,0.15,...%0.2,0.15,...      %2,2,...                       %C(4)+.001, 2,... %1/T1Lin, K(MCT4)
     2];    %0.5];%0.25];                      %1/25,]; %1/32,]; % 1/T1Lout,Flow_lacout, K(MCT1) Changed from 35.7 to 35
