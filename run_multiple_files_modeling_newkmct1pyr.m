function result = run_multiple_files_modeling_newkmct1pyr()
...
starting_entries = [13,6,11,9,8,8,18,17,12,15,11,2];%did final starting point analysis with these values[13,5,11,9,6,11,17,16,12,16];%[13,5,3,9,2,8,13,11,19]; %[13,11,11,10,6,8,15,13,12]; %[13,11,11,12,6,11,15,13,12]
...
for k = 1 : length(theFiles)
    ...
    [X,S_fit_dyn] = BioRx_kinetics_Pyrfit_starting_out_newkmct1pyr(input_vector,strcat(theFiles(k),"PyrFINALsmooth"));
    ...
    [C,S_fitdyn2] = BioRx_Pyr_Lacin_newkmct1pyr(my_array,X,strcat(theFiles(k),"LacinFINALsmooth")); %[round2_output,fit2,results2] = BioRx_kinetics_Lacin_NoT1Lin(my_array,round1_output,strcat(theFiles(k),"Lacincs"));
    lacin_table(k,:) = C;
    ...
    [E,Sfitdyn3] = BioRx_3mets_Lac2pks_new_kmct1pyr(my_array_2,X,C,strcat(theFiles(k),"LacexFINALsmooth")); %[fit3,results3] = BioRx_kinetics_Lac2pks_NoT1s(my_array_2,round1_output,round2_output,strcat(theFiles(k),"Lacexcs"));
    lacex_table(k,:) = E;
  drawnow; % Force display to update immediately.
end
%xlswrite("Pyr_summarycsmath.xlsx",pyr_table)
xlswrite("Lacin_summarynewkmct1pyr.xlsx",lacin_table)
xlswrite("Lacex_summarynewkmct1pyr.xlsx",lacex_table)
