function result = run_multiple_files_modeling_no_IF()
...
for k = 1 : length(theFiles)
    ...
    [X,S_fit_dyn] = BioRx_kinetics_Pyrfit_starting_out_recover_noIF(input_vector,strcat(theFiles(k),"PyrFINALsmoothnoIF"));
    ...
    [C,S_fitdyn2] = BioRx_Pyr_Lacin_recover_noIF(my_array,X,strcat(theFiles(k),"LacinFINALsmoothnoIF")); %[round2_output,fit2,results2] = BioRx_kinetics_Lacin_NoT1Lin(my_array,round1_output,strcat(theFiles(k),"Lacincs"));
    lacin_table(k,:) = C;
    ...
    [E,Sfitdyn3] = BioRx_3mets_Lac2pks_recover_noIF(my_array_2,X,C,strcat(theFiles(k),"LacexFINALsmoothnoIF")); %[fit3,results3] = BioRx_kinetics_Lac2pks_NoT1s(my_array_2,round1_output,round2_output,strcat(theFiles(k),"Lacexcs"));
...
xlswrite("Lacin_summary_noIF.xlsx",lacin_table)
xlswrite("Lacex_summary_noIF.xlsx",lacex_table)
xlswrite("AICnoIF.xlsx",AIC_vec)
