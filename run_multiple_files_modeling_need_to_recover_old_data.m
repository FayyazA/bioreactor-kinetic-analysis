...
for k = 1 : length(theFiles)
    ...
    input_vector = smooth(data_set.Pyr_area');
    [X,S_fit_dyn] = BioRx_kinetics_Pyrfit_starting_out_recover_old(input_vector,strcat(theFiles(k),"Pyr_othersolutionbcrm"));
    ...
    [C,S_fitdyn2] = BioRx_Pyr_Lacin_recover_old(my_array,X,strcat(theFiles(k),"Lacin_othersolutionbcrm")); %[round2_output,fit2,results2] = BioRx_kinetics_Lacin_NoT1Lin(my_array,round1_output,strcat(theFiles(k),"Lacincs"));
    lacin_table(k,:) = C;
    ...
    [E,Sfitdyn3] = BioRx_3mets_Lac2pks_recover_old(my_array_2,X,C,strcat(theFiles(k),"Lacex_othersolutionbcrm")); %[fit3,results3] = BioRx_kinetics_Lac2pks_NoT1s(my_array_2,round1_output,round2_output,strcat(theFiles(k),"Lacexcs"));
...
xlswrite("Lacin_summary_othersolutionbcrm.xlsx",lacin_table)
xlswrite("Lacex_summary_othersolutionbcrm.xlsx",lacex_table)
xlswrite("AICothersolutionbcrm.xlsx",AIC_vec)
