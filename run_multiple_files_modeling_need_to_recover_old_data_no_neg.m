function result = run_multiple_files_modeling_need_to_recover_old_data_no_neg()
...
for k = 1 : length(theFiles)
    ...
    [X,S_fit_dyn] = BioRx_kinetics_Pyrfit_starting_out_recover_old(input_vector,strcat(theFiles(k),"PyrFINALsmooth7param"));
    ...
    [C,S_fitdyn2] = BioRx_Pyr_Lacin_recover_old_no_negligible(my_array,X,strcat(theFiles(k),"LacinFINALsmooth7param")); %[round2_output,fit2,results2] = BioRx_kinetics_Lacin_NoT1Lin(my_array,round1_output,strcat(theFiles(k),"Lacincs"));
    lacin_table(k,:) = C;
    ...
    [E,Sfitdyn3] = BioRx_3mets_Lac2pks_recover_old_no_negligible(my_array_2,X,C,strcat(theFiles(k),"LacexFINALsmooth7param")); %[fit3,results3] = BioRx_kinetics_Lac2pks_NoT1s(my_array_2,round1_output,round2_output,strcat(theFiles(k),"Lacexcs"));
...
xlswrite("Lacin_summary_7param.xlsx",lacin_table)
xlswrite("Lacex_summary_7param.xlsx",lacex_table)
xlswrite("AIC7parameters.xlsx",AIC_vec)
