function [params_fit, Sfit, x1, sigma, objective_val] = fit_kPL(S, TR, flips, params_fixed, params_est, noise_level, plot_flag)
%%%% note, we were outputting ufit, not x1
...
params_default_est = [0.1, 1/25, 1/25, S(2,1)./sind(90)]; %S(2,1)./sind(22)
...
for i=1:size(S, 1)
    ...
    if any(y1(:) ~= 0)
        ...
        switch(fit_method)
            case 'ls'
                obj = @(var) (x2 - trajectories_frompyr(var, x1, Mzscale, params_fixed, TR)) .* Sscale(2,:); 
                [params_fit_vec,~,objective_val,~,~,~,J] = lsqnonlin(obj, params_est_vec, params_lb, params_ub, lsq_opts);
                % resulting Jacobian is a 4xNt, coresponding to what's in params_est_vec
                sigma = nlparci(params_fit_vec(i,1),objective_val,'jacobian',J(:,1));
        ...
        x1(1,:) = x1(1,:)  .* Sscale(1, :); % output the uncorrected pyr signal to plot
