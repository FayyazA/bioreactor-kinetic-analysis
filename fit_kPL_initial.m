function [params_fit, Sfit, ufit, objective_val] = fit_kPL_initial(S, TR, flips, params_fixed, params_est, noise_level, plot_flag)
...
% v2. with non-zero initial lactate signal
...
% initialize lac_t0
size_S = size(S); Nt = size_S(end);
[Sscale, Mzscale] = flips_scaling_factors(flips, Nt);
params_all{4} = 'lac_t0';
lac_t0_init_val = S(2,1)/Sscale(2,1);
params_est.lac_t0 = lac_t0_init_val;
params_default_est(4) = lac_t0_init_val;
params_default_lb(4) = 0;
params_default_ub(4) = Inf;
...
for i=1:size(S, 1)
    ...
    if any(y1 ~= 0)
        ...
        switch(fit_method)
            case 'ls'
                obj = @(var) (x2 - trajectories_frompyr_initial(var, x1, Mzscale, params_fixed, TR)).*Sscale(2,:);  % perform least-squares in signal domain
        ...
        [Sfit(i,:), ufit(i,:)] = trajectories_frompyr_initial(params_fit_vec(i,:), x1, Mzscale, params_fixed, TR);
