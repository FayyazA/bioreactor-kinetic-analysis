function [X,Sfit_dyn,lb,ub]=BioRx_Pyr_Lacin_recursive(S_dyn,A,filename,lb,ub)
...
X0 = [A(1),A(2),A(3),...    %1/T1Pyr, Kpl, Flow
    1/25, 0.45, .2]; %1/T1Lin, Klp=0, K(MCT4)=0.04
...
marker = "False";
%if abs(X(4) - lb(4)) <= 0.001 * X(4)
%lb(4) = lb(4) - 5 * lb(4);
%marker = "True";
%disp(4)
%end
%if abs(X(4) - ub(4)) <= 0.001 * X(4)
%ub(4) = ub(4) + 5 * ub(4);
%marker = "True";
%disp(4)
%end
if abs(X(5) - lb(5)) <= 0.001 * X(5)
lb(5) = lb(5) - 0.5 * lb(5);
marker = "True";
disp(5)
end
if abs(X(5) - ub(5)) <= 0.001 * X(5)
ub(5) = ub(5) + 0.5 * ub(5);
marker = "True";
disp(5)
end
% if abs(X(6) - lb(6)) <= 0.001 * X(6)
% lb(6) = lb(6) - 0.5 * lb(6);
% marker = "True";
% disp(6)
% end
% if abs(X(6) - ub(6)) <= 0.001 * X(6)
% ub(6) = ub(6) + 0.5 * ub(6);
% marker = "True";
% disp(6)
% end
...
if marker == "True"
    [X,Sfit_dyn,lb,ub]=BioRx_Pyr_Lacin_recursive(S_dyn,A,filename,lb,ub)
end
