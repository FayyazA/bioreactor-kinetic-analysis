function [X,Sfit_dyn,results,lb,ub]=BioRx_kinetics_Pyrfit_starting_out_recursive(S_dyn,filename,lb,ub)
...
X0 = [1/48,0.01,1,...    %1/T1P, Kpl,FlowPyr
...
marker = "False";
% if abs(X(1) - lb(1)) <= 0.001 * X(1)
% lb(1) = lb(1) * .99;
% marker = "True";
% end
% if abs(X(1) - ub(1)) <= 0.001 * X(1)
% ub(1) = ub(1) * 1.01;
% marker = "True";
% end
% if abs(X(2) - lb(2)) <= 0.001 * X(2)
% lb(2) = lb(2) * .99;
% marker = "True";
% end
% if abs(X(2) - ub(2)) <= 0.001 * X(2)
% ub(2) = ub(2) * 1.01;
% marker = "True";
% end
if abs(X(3) - lb(3)) <= 0.001 * X(3)
lb(3) = lb(3) - lb(3) * 10;
marker = "True";
end
if abs(X(3) - ub(3)) <= 0.001 * X(3)
ub(3) = ub(3) * 1.01;
marker = "True";
end
...
% end
if marker == "True"
[X,Sfit_dyn,results,lb,ub] = BioRx_kinetics_Pyrfit_starting_out_recursive(S_dyn,filename,lb,ub);
