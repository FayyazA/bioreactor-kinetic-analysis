...
% Inital conditions
    ...
    K  = [-C(1)-x(2)-x(3),0,0;... %Pyr=P(-R1P-kpl-Flow)+Lac_in(Klp) Replace flow by 0.5 and put KMCT1 in entry(1,1)
          ...
          0,x(6), -x(7)-x(8)] ;   %Lac_ex=Lac_in(K(MCT4))+Lac_out(-R1Lac_out-Flow-K(MCT1))   
    ...
    for k = 2:length(S_dyn)
       ...
       %                 0;...
       %                 0]; 
       Inpfunc(1,k) = -(Mest_dyn(1,k)-Mest_dyn(1,k-1)*exp((-x(1)-x(2)-x(3))*TR))* (x(1)+x(2)+x(3)) / (1-exp((-x(1)-x(2)-x(3))*TR));
...
lb = [1/51, .0001, 1E-8,...%[1/51, .0001, 1E-8,... % 1/T1Pyr, Kpl, Flow_pyr Flow_pyr used to be C(3)*0.9 1/T1P was C(1)-.001
 ...
 ub = [1/47, 0.5, 10.0,...%[1/47, 0.08, 10.0,... % 1/T1Pyr, Kpl, Flow_pyr Flow_pyr used to be C(3)*1.1 1/T1P was C(1)-.001
      C(4)+.001, 10, 10,...%C(4)+.001, 0.1, 10,... %1/T1Lin, Klp, K(MCT4)
